...
struct shortAxe: weapon {
    ...
    var name: String {return "\(self.cMaterial.rawMaterial.name) \(self.typeName)(\(self.cAxeHead.simpleDescription), \(self.cHaft.simpleDescription), \(self.cHandle.simpleDescription), \(self.cPommel.simpleDescription))"}
...
struct shortMace: weapon {
    ...
    var length: Int {var length = 0; for item in self.cList {length += item.length}; return length}
    var name: String {return "\(self.cMaterial.rawMaterial.name) \(self.typeName)(\(self.cMaceHead.simpleDescription), \(self.cHaft.simpleDescription), \(self.cHandle.simpleDescription), \(self.cPommel.simpleDescription))"}
    var grip: Int {return self.cHaft.grip}
}
...
struct sword: weapon, bladedWeapon {
    ...
    var cList: [weaponComponent] {return [self.cBlade, self.cGuard, self.cHandle, self.cPommel]}
    var length: Int {var length = 0; for item in self.cList {length += item.length}; return length}
    var name: String {return "\(self.cMaterial.rawMaterial.name) \(self.typeName)(\(self.cBlade.simpleDescription), \(self.cGuard.simpleDescription), \(self.cHandle.simpleDescription), \(self.cPommel.simpleDescription))"}
    var grip: Int {return self.cHandle.grip}
}
