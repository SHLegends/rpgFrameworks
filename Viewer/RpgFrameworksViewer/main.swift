...
start: while true {
    ...
    if let choice = startChoice {
        if choice == "1" {
            ...
            var foo = worldMap(); foo.canvasSetup(); foo.experimentalGen(drawSize: 1, branches: 1, branchLength: 1, searchSize: 2); foo.genContinent(1500, 2); foo.genContinent(700, 2); foo.genContinent(500, 2); foo.genContinent(200, 1); foo.genContinent(100, 1); foo.genContinent(50, 1); foo.genContinent(10, 1); foo.genContinent(100, 1); foo.genContinent(100, 1); foo.genContinent(100, 1); foo.genContinent(100, 1); foo.genContinent(100, 1); foo.experimentalGen(drawSize: 1, branches: 1, branchLength: 1, searchSize: 2); foo.fullMapScan(); foo.landMassDescription(); foo.printOut()
        } else if choice == "0" {
            break start
        } else {
            continue start
